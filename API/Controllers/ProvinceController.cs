...
namespace API.Controllers
{
    ...
    public class ProvinceController : ControllerBase
    {
        ...
        public async Task<ActionResult<ProvinceServiceResponse>> UpdateProvinceAsync(Guid id, ProvinceServiceInput input)
        {
            try
            {
                return Ok(await _provinceService.UpdateProvinceAsync(id, input)); //await _provinceService.UpdateProvinceAsync(id, input)
            }
        ...
        [HttpGet]
        public async Task<ActionResult<ProvinceServiceResponse>> GetAllProvinces()
        {
            try
            {
                return Ok(await _provinceService.GetAllProvinceAsync());
            }
            catch (Exception ex)
            {
                return StatusCode(StatusCodes.Status500InternalServerError, ex.Message);
